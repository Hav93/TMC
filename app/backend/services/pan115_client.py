        ...
        try:
            ...
            if clouddrive2_enabled:
                ...
                try:
                    ...
                    logger.error("❌ CloudDrive2 模块未安装")
                    return {
                        'success': False,
                        'message': 'CloudDrive2 模块未安装，请先安装 grpcio'
                    }
                except Exception as e:
                    logger.error(f"❌ CloudDrive2 上传异常: {e}")
                    import traceback
                    traceback.print_exc()
                    return {
                        'success': False,
                        'message': f'CloudDrive2 上传失败: {str(e)}'
                    }
            else:
                return {
                    'success': False,
                    'message': '上传功能需要启用 CloudDrive2，请设置环境变量 CLOUDDRIVE2_ENABLED=true'
                }
