...
async def activate_open_api(
    ...
    try:
        ...
        if token_result.get('success'):
            ...
            setattr(settings, 'pan115_token_expires_at', datetime.utcnow() + timedelta(seconds=expires_in))
            await db.commit()
    ...
    try:
        ...
        if not access_token and app_id and app_key:
            ...
            if token_result.get('success'):
                ...
                expires_in = token_result.get('expires_in', 7200)
                settings.pan115_access_token = access_token
                settings.pan115_token_expires_at = datetime.utcnow() + timedelta(seconds=expires_in)
                await db.commit()
    ...
    try:
        ...
        if result.get('status') == 'confirmed':
            ...
            if cookies and user_id:
                ...
                if access_token:
                    setattr(settings, 'pan115_access_token', access_token)
                    setattr(settings, 'pan115_token_expires_at', datetime.utcnow() + timedelta(seconds=7200))
                    logger.info(f"🔑 已保存access_token到数据库(有效期2小时)")
                else:
                    # 没有access_token时,设置cookies的预估过期时间为30天
                    setattr(settings, 'pan115_token_expires_at', datetime.utcnow() + timedelta(days=30))
