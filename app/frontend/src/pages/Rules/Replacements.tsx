  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text: string, record: ReplaceRule) => (
        <div>
          ...
          {record.is_regex && (
            <span style={{ color: colors.info, fontSize: '12px', marginLeft: '8px' }}>
              正则
            </span>
          )}
        </div>
    ...
    },
    {
      ...
      render: (priority: number) => (
        <span style={{ color: colors.textPrimary }}>{priority}</span>
      ),
    },
    {
      title: '状态',
      ...
      render: (is_active: boolean) => (
        <span className={is_active ? 'table-status-active' : 'table-status-inactive'}>
          {is_active ? '启用' : '禁用'}
        </span>
      ),
    },
          ...
          locale={{
            emptyText: (
              <TableEmpty 
                icon="🔄"
                title="暂无替换规则"
                description='点击"添加替换规则"开始设置文本替换'
              />
            )
          }}
          pagination={{
