...
export const systemApi = {
  ...
  status: async (): Promise<SystemStatus> => {
    return api.get<SystemStatus>(API_ROUTES.system.telegram.status);
  },

  // 获取统计数据
  stats: async (): Promise<Stats> => {
    return api.get<Stats>(API_ROUTES.system.stats);
  },

  // 重启Telegram客户端
  restart: async (): Promise<void> => {
    return api.post<void>(API_ROUTES.system.telegram.restart);
  },
  ...
  testCredentials: async (config: TelegramConfig): Promise<{
    ...
    return api.post(API_ROUTES.system.telegram.testCredentials, config);
  },

  // 获取当前配置
  getConfig: async (): Promise<Record<string, unknown>> => {
    return api.get<Record<string, unknown>>(API_ROUTES.system.config.current);
  },

  // 强制重新加载配置
  reloadConfig: async (): Promise<void> => {
    return api.post<void>(API_ROUTES.system.config.forceReload);
  },
  ...
  getConfigSyncStatus: async (): Promise<{
    ...
    return api.get(API_ROUTES.system.config.syncStatus);
  },
};
...
export const chatsApi = {
  // 获取聊天列表
  list: async (): Promise<{ chats: Chat[]; debug?: unknown }> => {
    return api.get<{ chats: Chat[]; debug?: unknown }>('/api/chats');
  },
